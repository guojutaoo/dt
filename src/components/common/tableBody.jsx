      ...
      {currentPage === 1 && (
        <tr key="0">
          ...
          <td scope="row" className="text-right">
              <FontAwesomeIcon id="icon" icon="plus" onClick={() => onCreate(today, day, timeStamp)}/>
          </td>
        </tr>
      )}
      {pastVerses.map(pastVerse => (
        <tr key={pastVerse}>
          ...
          <td className="text-right">
            ...
            {!showButton && (
              <React.Fragment>
                  <FontAwesomeIcon id="icon" icon="trash-alt" onClick={() => onDelete(pastVerse.id)}/>
                  <FontAwesomeIcon id="icon" icon="pencil-alt" onClick={() => onUpdate(pastVerse.verse)}/>
              </React.Fragment>
            )}
          </td>
        </tr>
